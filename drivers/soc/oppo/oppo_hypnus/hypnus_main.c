...
struct hypnus_data *get_hypnus_data(void)
{
	return &g_hypdata;
}
...
static int hypnus_cpufreq_thermal_notifier(struct notifier_block *nb, unsigned long val, void *data)
{
	...
	if (hypdata->forceflag == THERM_FORCE_FLAG) {
		...
		pr_debug("forceflag = %d, c_id = %d, max = %ld, min = %ld \n", hypdata->forceflag, cluster_id,
				max, min);
	}
...
long hypnus_ioctl_get_input_info(struct hypnus_data *pdata, unsigned int cmd, void *data)
{
	struct hypnus_input_info *input_info = data;
	unsigned long long now = ktime_to_ms(ktime_get_boottime());

	input_info->last_click_time_ms = (pdata->input_data).last_click;
	if(now > (pdata->input_data).last_click)
		input_info->last_click_internal_ms = now - (pdata->input_data).last_click;
	else
		input_info->last_click_internal_ms = 0;
...
int __init hypnus_init(void)
{
	...
	spin_lock_init(&pdata->lock);
        spin_lock_init(&pdata->scene_lock);
