...
#define IOCTL_HYPNUS_SET_STORAGE_SCALING \
	_IOW(HYPNUS_IOC_MAGIC, 0x25, struct hypnus_storagescaling_prop)
...
#define IOCTL_HYPNUS_GET_INPUT_INFO \
	_IOR(HYPNUS_IOC_MAGIC, 0x37, struct hypnus_input_info)
...
struct hypnus_storagescaling_prop {
    unsigned int storage_scaling;
};
...
struct hypnus_input_info {
	unsigned long long last_click_time_ms;
	unsigned long long last_click_internal_ms;
};
#endif
